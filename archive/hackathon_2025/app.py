...
if 'map_center' not in st.session_state:
    st.session_state['map_center'] = [center_point[0], center_point[1]]
...
with col_mode:
    mode = st.radio("📍 設定モード", ["スタート", "ゴール"], horizontal=True)
with col_msg:
    st.info(f"地図をクリックして **{mode}（{'緑' if mode=='スタート' else '赤'}）** を設置してください。")
