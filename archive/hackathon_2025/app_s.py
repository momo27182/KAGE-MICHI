...
def create_shadow_polygon(buildings_proj, dt_utc, center_point):
    ...
    polygons = []
    # 高速化: 不要な列を落としてイテレーション
    # 建物データが多い場合、ここがボトルネックになるため
    for geom, h in zip(buildings_proj.geometry, buildings_proj['height']):
    ...
    # ★高速化ポイント1: .simplify(0.5)
    # 0.5m以下の細かい凹凸を無視してデータ量を減らす
...
def solve_route(G_proj, orig, dest, all_shadows, sun_penalty):
    # ★高速化ポイント2: Prepared Geometry
    # 影データが巨大でも一瞬で判定できるように「準備」する
    prepared_shadows = prep(all_shadows) if all_shadows else None
    ...
    for u, v, k, data in G_proj.edges(keys=True, data=True):
        ...
        is_shaded = False
        # 高速化された prepared_shadows を使って判定
        if prepared_shadows and prepared_shadows.contains(mid):
            is_shaded = True
...
# 影計算は重いので、キャッシュするか「必要なときだけ計算」したいが
# Streamlitの構造上、ここで計算するのがシンプル。simplifyで高速化済み。
