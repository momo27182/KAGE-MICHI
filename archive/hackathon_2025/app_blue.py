...
with st.spinner('広域地図データ(半径1.7km)を読み込み中...'):
...
# ★デザイン変更: 地図を「CartoDB positron」（白ベース）に変更
...
if st.session_state['start_point'] and st.session_state['end_point']:
    ...
    if result_now:
        ...
        risk_level = "安全"
        risk_color = "#8090C0" # 安全時はテーマカラーに合わせる
        advice = "快適なルートです。"
        ...
        if temp_input >= 35: 
            if sunny_dist > 500:
                ...
                risk_level = "厳重警戒"
                risk_color = "#FFA500" # オレンジ
                advice = "極めて暑いです。必ず水分を持ってください。"
            else:
                risk_level = "警戒"
                risk_color = "#FFD700" # ゴールド
